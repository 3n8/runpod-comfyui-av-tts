...
AV_TTS_WORKFLOW_PATH = os.environ.get(
    "AV_TTS_WORKFLOW_PATH", "/workflows/ltx23_av_tts_api.json"
)
QWEN3_TTS_BASE_URL = os.environ.get(
    "QWEN3_TTS_BASE_URL", ""
).rstrip("/")
QWEN3_TTS_TIMEOUT = int(os.environ.get("QWEN3_TTS_TIMEOUT", "180"))
MAX_BASE64_VIDEO_MB = int(os.environ.get("MAX_BASE64_VIDEO_MB", "150"))
...
def validate_input(job_input):
    ...
    is_av_tts_request = "tts_text" in job_input

    # Validate 'workflow' in input. AV/TTS requests may use the bundled workflow.
    workflow = job_input.get("workflow")
    if workflow is None and not is_av_tts_request:
    ...
    # Optional single-image shorthand for the AV/TTS endpoint.
    if images is None and job_input.get("image") is not None:
        images = [{"name": "runpod_input.png", "image": job_input["image"]}]

    if is_av_tts_request and not images:
        return None, "AV/TTS requests require 'image' or 'images'"
    ...
    return {
        ...
        "av_tts": is_av_tts_request,
        "tts_text": job_input.get("tts_text"),
        "voice_id": job_input.get("voice_id") or os.environ.get("QWEN3_TTS_VOICE_ID"),
        ...
        "duration_padding": job_input.get("duration_padding", 0),
        "max_duration": job_input.get("max_duration"),
        "seed": job_input.get("seed"),
        "tts_audio": job_input.get("tts_audio"),
    }, None
...
def synthesize_qwen3_tts(text, voice_id):
    """Call the private Qwen3-TTS API and return MP3 bytes."""
    api_key = os.environ.get("QWEN3_TTS_API_KEY")
    if not api_key:
        raise ValueError("QWEN3_TTS_API_KEY is required for AV/TTS requests")
    if not QWEN3_TTS_BASE_URL:
        raise ValueError("QWEN3_TTS_BASE_URL is required for AV/TTS requests")
    if not voice_id:
        raise ValueError("voice_id or QWEN3_TTS_VOICE_ID is required for AV/TTS requests")
    if not text:
        raise ValueError("tts_text cannot be empty")

    endpoint = f"{QWEN3_TTS_BASE_URL}/v1/text-to-speech/{voice_id}"
    response = requests.post(
        ...
        timeout=QWEN3_TTS_TIMEOUT,
    )
    response.raise_for_status()
    return response.content


def write_job_audio(job_id, audio_bytes):
    os.makedirs(COMFY_INPUT_DIR, exist_ok=True)
    filename = f"{_safe_job_part(job_id)}_qwen_tts.mp3"
...
def load_av_tts_workflow_template():
    with open(AV_TTS_WORKFLOW_PATH, "r") as workflow_file:
        return json.load(workflow_file)


def prepare_av_tts_workflow(validated_data, job_id):
    audio_bytes = (
        _decode_base64_payload(validated_data["tts_audio"])
        if validated_data.get("tts_audio")
        else synthesize_qwen3_tts(validated_data["tts_text"], validated_data["voice_id"])
    )
    audio_filename, audio_path = write_job_audio(job_id, audio_bytes)
    audio_duration = probe_audio_duration(audio_path)

    max_duration = validated_data.get("max_duration")
    if max_duration is not None:
        audio_duration = min(audio_duration, float(max_duration))
    duration = audio_duration + float(validated_data.get("duration_padding") or 0)
    fps = int(validated_data["fps"])
    frame_count = int(math.ceil(duration * fps)) + 1

    workflow = load_av_tts_workflow_template()
    prompt = validated_data.get("prompt") or validated_data["tts_text"]

    workflow["269"]["inputs"]["image"] = validated_data["images"][0]["name"]
    workflow["319"]["inputs"]["value"] = prompt
    workflow["323"]["inputs"]["value"] = fps
    workflow["330"]["inputs"]["value"] = int(validated_data["width"])
    workflow["324"]["inputs"]["value"] = int(validated_data["height"])
    workflow["331"]["inputs"]["value"] = duration
    workflow["332"]["inputs"]["start_index"] = float(validated_data.get("audio_start") or 0)
    workflow["345"]["inputs"]["audio_file"] = f"input/{audio_filename}"
    workflow["341"]["inputs"]["filename_prefix"] = f"video/{_safe_job_part(job_id)}_comfyui_av_tts"

    if validated_data.get("negative_prompt") and "314" in workflow:
        workflow["314"]["inputs"]["text"] = validated_data["negative_prompt"]
    if validated_data.get("seed") is not None:
        seed = int(validated_data["seed"])
        for node_id in ("285", "286"):
            if node_id in workflow and "noise_seed" in workflow[node_id]["inputs"]:
                workflow[node_id]["inputs"]["noise_seed"] = seed

    logger.info(
        "Prepared AV/TTS workflow: audio_duration=%.3fs, duration=%.3fs, fps=%d, frames=%d",
        audio_duration,
        duration,
        fps,
        frame_count,
...
def get_file_data(filename, subfolder, file_type):
    ...
    """
    print(
        f"worker-comfyui - Fetching output data: type={file_type}, subfolder={subfolder}, filename={filename}"
    )
...
def get_image_data(filename, subfolder, image_type):
    return get_file_data(filename, subfolder, image_type)


def encode_output_file(filename, file_bytes, output_kind):
    size_mb = len(file_bytes) / (1024 * 1024)
    if output_kind == "video" and size_mb > MAX_BASE64_VIDEO_MB:
        raise ValueError(
            f"Video {filename} is {size_mb:.1f} MB, above MAX_BASE64_VIDEO_MB={MAX_BASE64_VIDEO_MB}"
        )
    return {
        "filename": filename,
        "type": "base64",
...
def handler(job):
    ...
    if validated_data.get("av_tts"):
        try:
            workflow = prepare_av_tts_workflow(validated_data, job_id)
        except Exception as e:
            print(f"worker-comfyui - Error preparing AV/TTS workflow: {e}")
            print(traceback.format_exc())
            return {"error": f"Error preparing AV/TTS workflow: {e}"}
        ...
        for node_id, node_output in outputs.items():
            ...
            for media_key in ("videos", "gifs"):
                if media_key not in node_output:
                    continue
                print(
                    f"worker-comfyui - Node {node_id} contains {len(node_output[media_key])} {media_key}"
                )
                for media_info in node_output[media_key]:
                    ...
                    if media_type == "temp":
                        print(
                            f"worker-comfyui - Skipping media {filename} because type is 'temp'"
                        )
                        continue
                    if not filename:
                        warn_msg = f"Skipping media in node {node_id} due to missing filename: {media_info}"
                        print(f"worker-comfyui - {warn_msg}")
                        errors.append(warn_msg)
                        continue

                    media_bytes = get_file_data(filename, subfolder, media_type)
                    if not media_bytes:
                        errors.append(
                            f"Failed to fetch media data for {filename} from /view endpoint."
                        )
                        continue

                    try:
                        video_data.append(encode_output_file(filename, media_bytes, "video"))
            ...
            other_keys = [
                k for k in node_output.keys() if k not in ("images", "videos", "gifs")
            ]
            if other_keys:
                warn_msg = (
    ...
    if not output_data and not video_data and errors:
        print(f"worker-comfyui - Job failed with no output files.")
        return {
            "error": "Job processing failed",
            "details": errors,
        }
    elif not output_data and not video_data and not errors:
        print(
    ...
    print(
        f"worker-comfyui - Job completed. Returning {len(output_data)} image(s), {len(video_data)} video(s)."
    )
    return final_result
