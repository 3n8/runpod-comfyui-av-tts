...
# First target is 48GB+ NVIDIA, so default to keeping models on GPU. Override
# with COMFYUI_ARGS if a smaller GPU needs lowvram/reserve-vram tuning.
...
if [ "$SERVE_API_LOCALLY" == "true" ]; then
    python -u /comfyui/main.py --disable-auto-launch --disable-metadata --listen --verbose "${COMFY_LOG_LEVEL}" --log-stdout ${COMFYUI_ARGS} &
    echo $! > "$COMFY_PID_FILE"

    echo "worker-comfyui: Starting RunPod Handler"
    python -u /handler.py --rp_serve_api --rp_api_host=0.0.0.0
else
    python -u /comfyui/main.py --disable-auto-launch --disable-metadata --verbose "${COMFY_LOG_LEVEL}" --log-stdout ${COMFYUI_ARGS} &
    echo $! > "$COMFY_PID_FILE"
