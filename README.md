# runpod-comfyui-av-tts

RunPod Serverless worker for ComfyUI audio-video text-to-speech generation.
It takes an input image and TTS text, calls a private Qwen3-TTS API inside the
worker, computes video length from the generated audio duration, runs an
LTX-2.3 image/audio-to-video workflow, and returns the MP4 as base64.
...
```json
{
  "input": {
    ...
    "tts_text": "Hello, this is a short test.",
    "prompt": "A character speaks naturally to the camera.",
...
- `QWEN3_TTS_API_KEY`
- `QWEN3_TTS_BASE_URL`
- `QWEN3_TTS_VOICE_ID`, unless each request supplies `voice_id`

Optional endpoint environment variables:

- `MAX_BASE64_VIDEO_MB`, default `150`

For local `docker-compose` testing, copy `.env.example` to `.env` and set the
same values there. `.env` is ignored and must not be committed. On RunPod, use
the endpoint/template environment variable GUI instead of `.env`.

See `docs/runtime-configuration.md` for the full runtime configuration notes.
...
```json
{
  "output": {
    "videos": [
      {
        "filename": "JOBID_comfyui_av_tts_00001.mp4",
        "type": "base64",
        "data": "AAAA..."
      }
    ]
...
See `docs/container-requirements.md` for the model/node inventory and the
NVIDIA/LTX workflow decisions.

# Upstream worker-comfyui notes
